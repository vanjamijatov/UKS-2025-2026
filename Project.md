...
properly maintaining [the web-application (simplified *DockerHub*)](https://docs.google.com/document/d/1DRjjjZDnjWoZiU_oyHSs4har-wkZvMMlTIxoEg_3m7o/edit?usp=sharing) life cycle from its early beginning until deploying in production.
The total number of points that can be achieved by successfully finishing the project is 60. 
The following list covers the items that form the project:
- A Class Diagram that describes the model of you application.
- Implementation of [the web application (simplified *DockerHub*)](https://docs.google.com/document/d/1DRjjjZDnjWoZiU_oyHSs4har-wkZvMMlTIxoEg_3m7o/edit?usp=sharing) 
- Implementation of the tests:
  - Unit tests - Each component (a class) should be covered by unit tests.
  - Integration tests - The server side of the application should be covered by integration tests.
    Mock up the client which sends the HTTP requests.
- Git Tool Usage - The git tool is intended to be used by respecting the [GitFlow Workflow](https://www.atlassian.com/git/tutorials/comparing-workflows/gitflow-workflow). 
  More information can be found in the corresponding section below.
- GitHub Platform Usage - To track down the project development, GitHub platform should be used. 
  More information can be found in the corresponding section below.
-  Software Configuration Management: 
  - The application should be containerized in the corresponding image. 
  - Container Orchestration - It is expected to orchestrate the following containers (more containers can be introduced in the system):
  ...
  - Usage of CI/CD tools - From the early beginning of the application development, students should use use a CI/CD Tool.
    It is expected to provide the following build plans:
    - A build plan triggered when pull request is created on the *develop* branch. This build plan is responsible to run all automated tests and provide the output 
      whether it is safe to merge the pull request.
    - A build plan triggered when new version of the app is pushed on the main/master branch.
